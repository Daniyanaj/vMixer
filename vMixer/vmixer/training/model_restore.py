...
import importlib
import pkgutil
from vmixer.training.network_training.vmixerTrainer import vmixerTrainer
from vmixer.training.network_training.vmixerTrainer_synapse import vmixerTrainer_synapse
from vmixer.paths import network_training_output_dir, preprocessing_output_dir, default_plans_identifier
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None,folder=None):
    """
    This is a utility function to load any vmixer trainer from a pkl. It will recursively search
    vmixer.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    is specified, it will furthermore load the checkpoint file in train/test mode (as specified by train).
    The pkl file required here is the one that will be saved automatically when calling vmixerTrainer.save_checkpoint.
    :param pkl_file:
    :param checkpoint:
    :param train:
    ...
    if 'nnUNet' in name:
        name=name.replace('nnUNet','vmixer')
    if len(init)>10:
        init=list(init)
        del init[2]
        del init[-2]
    search_in = join(vmixer.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="vmixer.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in vmixer.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within vmixer.trainig.network_training (search is recursive)."
                           "\nDebug info: \ncheckpoint file: %s\nName of trainer: %s " % (checkpoint, name))
    assert issubclass(tr, vmixerTrainer) or issubclass(tr, vmixerTrainer_synapse), "The network trainer was found but is not a subclass of vmixerTrainer.Please make it so!"

    # this is now deprecated
    """if len(init) == 7:
        print("warning: this model seems to have been saved with a previous version of vmixer. Attempting to load it "
...
if __name__ == "__main__":
    pkl = "/home/fabian/PhD/results/vmixerV2/vmixerV2_3D_fullres/Task004_Hippocampus/fold0/model_best.model.pkl"
    checkpoint = pkl[:-4]
    train = False
